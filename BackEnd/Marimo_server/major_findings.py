...
@app.cell(hide_code=True)
def _(pkl):
    with open('./scripts/pkl_files/ahp_tabs.pkl', 'rb') as _f:
        ahp = pkl.load(_f)
...
def _(mo):
    mo.md(
        r"""
    ---
    **<span style='color: brown'>Utilizing Catboost for Initial Stage Probability Calculation</span>**
...
@app.cell(hide_code=True)
def _(pkl):
    with open('scripts/pkl_files/catboos.pkl', 'rb') as _f:
        catboost = pkl.load(_f)
    catboost
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.image('alternative_prognosis data/miRNA.png')
    return
...
@app.cell(hide_code=True)
def _(pkl):
    with open('scripts/pkl_files/cox.pkl', 'rb') as _f:
        cox = pkl.load(_f)
    cox
    return
